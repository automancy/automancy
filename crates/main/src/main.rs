...
use color_eyre::{config::HookBuilder, owo_colors};
use kira::{
...
fn write_crash_msg<P: AsRef<Path>>(buffer: &mut impl Write, file_path: P) -> std::fmt::Result {
    ...
    writeln!(
        buffer,
        "To help diagnose the problem, you can send us a crash report."
    )?;
...
fn shutdown_game(automancy: &mut Automancy, event_loop: &ActiveEventLoop) {
    event_loop.exit();

    {
        let game = automancy.game_state.game.clone();
        let game_handle = automancy
            .game_state
            .game_handle
            .take()
            .expect("game handle needs to be set");

        automancy.game_state.tokio.block_on(async {
    ...
    fn window_event(
        ...
        if !self.closed {
            ...
            */
            ...
            if self.closed {
                log::info!("Window set to be closed. Shutting the game down now.");
                shutdown_game(self, event_loop);
            }
        } else {
...
fn main() -> anyhow::Result<()> {
...

    {
        ...
        panic::set_hook(Box::new(move |info| {
            ...
            let _ = writeln!(
                dialog_text,
                "Do you want to open the report file right now?"
            );
