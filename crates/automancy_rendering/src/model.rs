use automancy_data::{
    id::ModelId,
    math::{Float, Matrix4, Quat, Transform, Vec3, Vec4},
};
...
pub struct ModelManager {
    ...
    pub global_mesh_ids: HashMap<(ModelId, MeshId), GlobalMeshId>,

    pub model_mesh_ids: HashMap<ModelId, Vec<MeshId>>,
    pub global_model_mesh_ids: HashMap<ModelId, Vec<GlobalMeshId>>,
...
impl ModelManager {
    fn parse_gltf_model(
    ...
    ) -> (Vec<Option<Mesh>>, HashMap<MeshId, AnimationChannel>) {
        ...
        for animation in document.animations() {
            for channel in animation.channels() {
                ...
                let mesh_id = MeshId::from(target as u16);
                ...
                if let Some((inputs, outputs)) = reader.read_inputs().zip(reader.read_outputs()) {
                    match outputs {
                        ReadOutputs::Translations(outputs) => {
                            for (input, output) in inputs.zip(outputs) {
                                read_values.push((
                                    input,
                                    AnimationOutput::Translation(Vec3::from_slice(&output)),
                                ));
                            }
                        }
                        ReadOutputs::Scales(outputs) => {
                            for (input, output) in inputs.zip(outputs) {
                                read_values.push((
                                    input,
                                    AnimationOutput::Scale(Vec3::from_slice(&output)),
                                ));
                            }
                        }
                        ReadOutputs::Rotations(outputs) => {
                            for (input, output) in inputs.zip(outputs.into_f32()) {
                                read_values.push((
                                    input,
                                    AnimationOutput::Rotation(Quat::from_vec4(Vec4::from_slice(
                                        &output,
                                    ))),
                                ));
                            }
                    ...
                    for (input, output) in read_values {
                        if let Some((_, transform)) =
                            transforms.iter_mut().find(|(v, _)| *v == input)
                        {
                            *transform = output.apply_to_transform(*transform);
                        } else {
                            transforms
                                .push((input, output.apply_to_transform(Transform::default())));
                        }
                    }
                    ...
                    animations.insert(
                        ...
                        AnimationChannel {
                            ...
                            keyframes: transforms
                                .into_iter()
                                .map(|(v, transform)| (v, Matrix4::from(transform)))
                                .collect(),
                        },
                    );
                }
            ...
            for (model_id, (meshes, animations)) in models {
                for mut mesh in meshes.into_iter().flatten() {
...

                    {
                        ...
                        assert_eq!(
                            self.global_mesh_ids
                                .insert((model_id, mesh.id), global_mesh_id),
                            None,
                        );
                        mesh_count += 1;

                        self.mesh_metadata
                            .insert(global_mesh_id, std::mem::take(&mut mesh.metadata));
                ...
                for (mesh_id, animation) in animations {
                    ...
                    assert_eq!(
                        self.animation_channels_map.insert(global_id, animation),
                        None
                    );
                }
            }
