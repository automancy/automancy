...
use crate::data::{
    GpuAnimationMatrixData, GpuDrawInstance, GpuGameLightingUniformData, GpuGameUniformData,
    GpuModelMatrixData, GpuPostProcessingUniformData, GpuVertex, GpuWorldMatrixData,
};
...
impl GameRenderTextures {
    pub fn new(device: &wgpu::Device, config: &wgpu::SurfaceConfiguration) -> Self {
        ...
        let lighting_surface_texture = device.create_texture(&surface_desc);
        let post_processing_surface_texture = device.create_texture(&surface_desc);
        let fxaa_surface_texture = device.create_texture(&surface_desc);
...
impl GamePipeline {
    pub fn new<
        const MODEL_MATRIX_LEN: usize,
        const WORLD_MATRIX_LEN: usize,
        const ANIMATION_MATRIX_LEN: usize,
    >(
        device: &wgpu::Device,
        ...
        let model_matrix_data_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: mem::size_of::<[GpuModelMatrixData; MODEL_MATRIX_LEN]>() as u64,
        ...
        let world_matrix_data_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: mem::size_of::<[GpuWorldMatrixData; WORLD_MATRIX_LEN]>() as u64,
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        ...
        let animation_matrix_data_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            ...
            size: mem::size_of::<[GpuAnimationMatrixData; ANIMATION_MATRIX_LEN]>() as u64,
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        ...
        let opaque_instance_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Opaque Game Instance Buffer"),
            contents: &[],
            usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
        });
        let non_opaque_instance_buffer =
            device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("Non-opaque Game Instance Buffer"),
                contents: &[],
                usage: wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
            });

        let opaque_indirect_draw_command_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("Opaque Game Indirect Draw Buffer"),
            size: mem::size_of::<[wgpu::util::DrawIndexedIndirectArgs; 256]>() as u64,
            usage: wgpu::BufferUsages::INDIRECT | wgpu::BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
        let non_opaque_indirect_draw_command_buffer =
            device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("Non-opaque Game Indirect Draw Buffer"),
                size: mem::size_of::<[wgpu::util::DrawIndexedIndirectArgs; 256]>() as u64,
                usage: wgpu::BufferUsages::INDIRECT | wgpu::BufferUsages::COPY_DST,
                mapped_at_creation: false,
            });
...
impl GameLightingPipeline {
    pub fn new(
        ...
        let bind_group_layout_samplers =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
    ...
    pub fn new(
        ...
        let game_lighting_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(
                resource_man.shaders["game_lighting"].to_string().into(),
            ),
        });
        ...
        let texture_sample_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(
                resource_man.shaders["texture_sample"].to_string().into(),
            ),
        });
...
impl GameRenderResources {
    pub fn new(
        ...
        let normal_texture = render_textures
            .normal_texture
            .create_view(&wgpu::TextureViewDescriptor::default());
        let model_pos_texture = render_textures
            .model_pos_texture
            .create_view(&wgpu::TextureViewDescriptor::default());
        ...
        let post_processing_surface_texture = render_textures
            .fxaa_surface_texture
            .create_view(&wgpu::TextureViewDescriptor::default());
        let _fxaa_surface_texture = render_textures
            .fxaa_surface_texture
            .create_view(&wgpu::TextureViewDescriptor::default());

        let game_pipeline = GamePipeline::new::<2048, 1, 2048>(device, global_res);
...
impl PresentResources {
    pub fn new(
        ...
        Self {
            game_gui_compose_pipeline: ComposePipeline::new(
                device,
                config,
                global_res,
                ComposePipelineArgs {
...
impl RenderResources {
    ...
    pub async fn new(
        ...
        let present_res =
            PresentResources::new(&device, &config, &global_res, &main_game_res, &gui_res);
