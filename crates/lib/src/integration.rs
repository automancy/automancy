...
pub mod util {
    ...
    pub(super) fn convert_game_input_event(
        ...
        if let Some(event) = window_event {
            match event {
                WindowEvent::MouseWheel { delta, .. } => {
                    result = match delta {
                        MouseScrollDelta::PixelDelta(delta) => {
                            let delta = Vec2::new(
                                delta.x as f32 * sensitivity / viewport_size.x,
                                delta.y as f32 * sensitivity / viewport_size.y,
                            );

                            GameInputEvent::MouseWheel { delta }
                        }
                        MouseScrollDelta::LineDelta(x, y) => {
                            let delta = Vec2::new(*x * sensitivity / 24.0, *y * sensitivity / 24.0);
                ...
                WindowEvent::ModifiersChanged(modifier) => {
                    result = GameInputEvent::ModifierChanged {
                        modifier: *modifier,
                    };
                }
                ...
                WindowEvent::KeyboardInput { event, .. } => {
                    result = GameInputEvent::KeyboardEvent {
                        event: event.clone(),
                    }
                }
                _ => {}
...
pub fn on_event(
    ...
    match &event {
        Event::WindowEvent {
            ...
            // game shutdown
            log::info!("Window close event received! Setting game to close.");
            *close_window = true;
            return Ok(());
        }
        Event::WindowEvent { event, window_id } if window_id == &render.res.window.id() => {
            match event {
                WindowEvent::RedrawRequested => {
                    ...
                    let result = util::render(close_window, game_state, render_state, render);
    ...
    if window_event.is_some() || device_event.is_some() {
        ...
        game_state
            .input_handler
            .update(util::convert_game_input_event(
                window_event,
                device_event,
                window_util::window_size(&render.res.window),
