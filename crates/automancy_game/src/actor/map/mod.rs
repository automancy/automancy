...
impl fmt::Display for GameMapId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            GameMapId::MainMenu => f.write_str("<main menu>"),
...
pub mod serialize {
    ...
    use crate::{
        ...
        persistent::ron::ron_options,
        resources::{ResourceManager, error::push_err, format::FormatContext},
    };
    ...
    pub fn read_map(
        ...
        let decoded: ron::error::SpannedResult<GameMapRaw> = match map_id {
            GameMapId::SaveFile(name) => {
                ...
                let decoder =
                    zstd::Decoder::with_buffer(io::BufReader::with_capacity(MAP_BUFFER_SIZE, file))
                        .unwrap();

                ron::de::from_reader(decoder)
            }
            GameMapId::MainMenu => {
                ron::de::from_reader(zstd::Decoder::with_buffer(MAIN_MENU_MAP).unwrap())
            }
            GameMapId::Debug => unreachable!(),
        };
        ...
        match decoded {
            ...
            Err(e) => {
                ...
                push_err(
                    ...
                    &FormatContext::from([("map_name", Formattable::display(&map_id))].into_iter()),
                    resource_man,
                );
    ...
    pub fn read_map_data(
        resource_man: &ResourceManager,
        map_id: &GameMapId,
    ) -> anyhow::Result<(GameMapDataRaw, Option<SystemTime>)> {
        let mut mtime = None;
        ...
        match decoded {
            Ok(v) => Ok((v, mtime)),
            Err(e) => {
                log::error!("Error loading map {map_id}, in reading map data: serde: {e:?}");
    ...
    pub fn load_map(
        ...
        map_id: &GameMapId,
    ) -> anyhow::Result<(FlatTiles, GameMapData)> {
