            ...
            GameMsg::LoadMap(map_id, reply) => {
                match std::mem::take(&mut state.game_state) {
                    GameState::Running(mut game_data) | GameState::Paused(mut game_data) => {
                        for tile in std::mem::take(&mut game_data.map.tiles)
                            .into_inner()
                            .into_values()
                        {
                            ...
                            let tiles = multi_call_iter(
                                ...
                                coords.into_iter().flat_map(|coord| {
                                    game_data
                                        ...
                                        .tiles
                                        .get(&coord)
                                        .map(|tile| ((coord, tile.id), tile.handle.clone()))
                                }),
                        ...
                        GameMsg::GetAllRenderCommands {
                            ...
                            match commands {
                                Ok(mut commands) => {
                                    ...
                                    reply.send([
                                        std::mem::take(&mut game_data.cleanup_render_commands),
                                        commands,
                                    ])?;
                                }
                                Err(err) => {
