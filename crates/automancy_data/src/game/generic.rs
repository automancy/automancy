...
pub mod deserialize {
    ...
    pub enum DatumStr {
        ...
        Id(StrId),
        VecId(Vec<StrId>),
        VecOffsetCoord(Vec<[Int; 2]>),
        SetId(Vec<StrId>),
        TileMap(Vec<(TileCoord, StrId)>),
        TileMapOffsetCoord(Vec<([Int; 2], StrId)>),
    ...
    impl DatumStr {
        pub fn into_datum(
            ...
            Ok(match self {
                ...
                DatumStr::VecOffsetCoord(v) => {
                    Datum::VecCoord(v.iter().map(|v| TileCoord::from_offset_coord(*v)).collect())
                }
                ...
                DatumStr::TileMapOffsetCoord(v) => Datum::TileMap(
                    parse_map_item_id(
                        v.into_iter()
                            .map(|(coord, id)| (TileCoord::from_offset_coord(coord), id)),
                        interner,
                        fallback_namespace,
                    )
                    .try_collect()?,
