...
use crate::math::{Float, Matrix3, Matrix4, Vec2, Vec3, Vec4, consts};

pub const WORLD_PLANE_Z: Float = 0.0;
pub const WORLD_FORWARD_DIR: Vec3 = Vec3::new(0.0, 0.0, 1.0);
pub const WORLD_NORMAL: Vec3 = Vec3::new(
    -WORLD_FORWARD_DIR.x,
    -WORLD_FORWARD_DIR.y,
    -WORLD_FORWARD_DIR.z,
);
...
pub fn projection(aspect: Float) -> Matrix4 {
    Matrix4::perspective_lh_zo(fov(), aspect, z_near(), z_far())
}
...
#[inline]
#[must_use]
fn view_plane_normal(camera_pos: Vec3) -> Vec3 {
...
pub fn normalized_to_world(pos: Vec2, aspect: Float, camera_pos: Vec3) -> Vec3 {
    ...
    let matrix =
        camera_view(Vec3::new(0.0, 0.0, camera_pos.z)).inverted() * projection(aspect).inverted();
...
#[inline]
#[must_use]
pub fn viewport_bounding_rect_in_world(viewport_size: Vec2, camera_pos: Vec3) -> (Vec2, Vec2) {
