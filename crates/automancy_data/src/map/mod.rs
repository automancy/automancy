...
pub static MAP_INFO_EXT: &str = "ron";
...
pub struct GameMap {
    ...
    pub tiles: Tiles,
    /// The map's info.
    pub info: Arc<Mutex<GameMapInfo>>,
}
...
impl GameMap {
    ...
    pub fn info(opt: &LoadMapOption) -> Option<PathBuf> {
        GameMap::path(opt).map(|v| v.join("info").with_extension(MAP_INFO_EXT))
    }
    ...
    pub fn read_info(
        ...
        opt: &LoadMapOption,
    ) -> Result<(GameMapInfoRaw, Option<SystemTime>), bool> {
        let mut time = None;

        let decoded: SpannedResult<GameMapInfoRaw> = match opt {
    ...
    pub fn read_map(
        resource_man: &ResourceManager,
        opt: &LoadMapOption,
    ) -> Result<GameMapRaw, bool> {
        let decoded: SpannedResult<GameMapRaw> = match opt {
            LoadMapOption::FromSave(name) => {
    ...
    pub async fn load(
        ...
        Ok((
            Self {
                opt: opt.clone(),
                tiles,
                info: Arc::new(Mutex::new(GameMapInfo {
                    save_time,
