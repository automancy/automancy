...
def main():
    ...
    pathAttrs = [path.attrib for path in root.iter("{http://www.w3.org/2000/svg}path")]
    styles = []
    for attrs in pathAttrs:
        styles.append(dict(
            map(
                lambda p: p.split(":"),
                (attrs.get("style") or "ihate:python").split(";"),
            )
        ))
    total = float(len(pathAttrs))
    ...
    for [index, obj] in enumerate(curves):
        ...
        new_obj.delta_location.z = (index / total) / 64.0 + 0.005
        alpha = styles[index].get("fill-opacity")
        if alpha:
